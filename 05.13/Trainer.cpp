...
void Trainer::battle(Creature & enemy)
{
	...
	do {
		...
		} while (!((idx <= pokemons[selectedIndex].getcount()) && (idx > 0))); 	atk = pokemons[selectedIndex].getAbility(idx-1).use(pokemons[selectedIndex], enemy); enemy.setHp(enemy.getHp() - atk); break; //TODO: Hogy adjam át, hogy ne legyen rossz?
		case 'S': list(); 
			do { cout << "Index of desired pokemon(choose an eligible one: "; cin >> idx; if (cin.fail()) throw ios_base::failure("String!=Number");
			} while (!((idx <= pokemoncount) && (idx > 0)));
		...
		if (alivePokemons())
		{
			if (pokemons[selectedIndex].getHp() <= 0.0)
			{
				...
				} while (!((idx <= pokemoncount) && (idx > 0))); switchtoIndex(idx);
			}
		}
		else
		{
...
void Trainer::battle(Pokemon & enemy)
{
	...
	do {
		...
		{
		case 'H': usePotion(); break;
		case 'M': pokemons[selectedIndex].listAbility(); do { cout << "Index of desired move: " << endl; cin >> idx; if (cin.fail()) throw ios_base::failure("String!=Number"); } while (!((idx <= pokemons[selectedIndex].getcount()) && (idx > 0))); 	atk = pokemons[selectedIndex].getAbility(idx - 1).use(pokemons[selectedIndex], enemy); enemy.setHp(enemy.getHp() - atk); break; //TODO: Hogy adjam át, hogy ne legyen rossz?
		...
		} while (!((idx <= pokemoncount) && (idx > 0))); switchtoIndex(idx); break;
		case 'R': cout << "You cannot retreat now" << endl; break;
