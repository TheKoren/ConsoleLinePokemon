...
int main() {
	...
	try {
		do {
			...
			} control[currentnode].Print(); WAIT break;
			...
			case 'H': Player.list(); cin >> dice; if (cin.fail()) {  throw ios_base::failure("Char!=num");}Player.usePotion(dice); WAIT break;
			case 'X': cout << "Are you sure, you want to quit? X-Let me gooo | N-Nope" << endl; cin >> choice; if (choice == 'X') { choice = 'X'; WAIT break; }
					  else if (choice == 'N') { choice = 'Y'; WAIT break; }
			case 'P': Player.list(); WAIT break;
			default:  cout << "Wrong Input" << endl; WAIT break;
			}
		} while (choice != 'X' || !(Player.alivePokemons()));
	}
	catch (ios_base::failure&ex) { cin.clear(); cout << ex.what(); }
