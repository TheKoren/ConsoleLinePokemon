...
int main() {
	...
	catch (exception&ex) { cout << ex.what(); exit(1); } 
	...
	cout << "Welcome! Type your name: "; getline(cin, name); Trainer Player(name, 10, 2, 0, 0); 
	...
	try {
		do {
			...
			case 'h':;
			case 'H': 
				Player.list(); cin >> dice; if (cin.fail()) {  throw ios_base::failure("Char!=num");}Player.usePotion(dice); Sleep(500); cout << "."; Sleep(500); cout << "." << endl; break;
			case 'x':;
