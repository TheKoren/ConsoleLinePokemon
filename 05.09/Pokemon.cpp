...
double Move::use(Pokemon & owner, Creature & enemy)
{
	...
	if (this->own == Neutral)
	{
		if (rand() % 1)
		{
			cout << owner.getName() << "causes " << owner.getBaseDmg()*1.1 << "damage on " << enemy.getName() << endl;
			return owner.getBaseDmg()*1.1;
		}
		else
		{
			cout << owner.getName() << "causes " << owner.getBaseDmg()*0.9 << "damage on " << enemy.getName() << endl;
			return owner.getBaseDmg()*0.9;
		}
	}
	if (enemy.getBaseDmg() > owner.getBaseDmg())
	{
		cout << owner.getName() << "causes " << this->failfaktor*owner.getBaseDmg() << "damage on " << enemy.getName() << endl;
		cout << "It's not very effective." << endl;
		return this->failfaktor*owner.getBaseDmg();
	}
	else
	{
		cout << owner.getName() << "causes " << this->succfaktor*owner.getBaseDmg() << "damage on " << enemy.getName() << endl;
